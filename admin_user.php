           ...
           <div class="row">
            <div class="col-md-8">
              <div class="card">
                ...
                <div class="card-body">
                  <form action="../Project/validations.php" method="POST">
                    <div class="row">
                      <div class="col-md-5">
                      ...
                      <div class="col-md-4">
                        <div class="form-group">
                          ...
                          <input type="email" class="form-control" 
                            value=" <?php
                                $con = mysqli_connect("localhost","root","","wdl");     //making connection(not needed)
                               $q = mysqli_query($con,"SELECT useremail from user WHERE username = '".$_SESSION['user']['username']."'");     //selecting Profile image of session user from database and storing in $q
                               while($row = mysqli_fetch_assoc($q)){                                                                                //checking if user has set any profile image or not
                               if($_SESSION['user']['useremail']==""){                                     //if the user haven't uploaded any proile image
                              echo "Email";                   //echo the default image
                                }
                              else{
                                echo $_SESSION['user']['useremail'];                   //echo the profile image
                              }
                            }
                             ?>" 
                          disabled>
            ...
            $con = mysqli_connect("localhost","root","","wdl");
             $q = mysqli_query($con,"SELECT Profile_image from user WHERE username = '".$_SESSION['user']['username']."'"); 
            while($row = mysqli_fetch_assoc($q)){
                      
                    if($_SESSION['user']['Profile_image']==""){
                      echo "<img src='assets/faces/default.png' alt='Profile Pic'>";                   //echo the profile image
                    }
                    else{
                         echo "<img src='assets/faces/".$_SESSION['user']['Profile_image']."' alt='Profile Pic'>";                   //echo the profile image
        ...
        <form class="form form-vertical" action="" method="post" enctype="multipart/form-data">  
          ...
          <div class="kv-avatar-hint">
            <small>Select file 1500 KB</small>
                ...
                <div class="card-body">
                  <center>
                  <h6 class="card-category text-gray"  style="color: #000000;"><?php echo ucfirst($_SESSION['user']['user_type']); ?></h6>
                  <h4 class="card-title">
                       <?php  if (isset($_SESSION['user'])) : ?>
          <strong><?php echo $_SESSION['user']['username']; ?></strong>
